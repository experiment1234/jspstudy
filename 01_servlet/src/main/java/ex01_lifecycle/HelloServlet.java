 ...
 * 3.javax.servlet.httpServlet 클래스를 상속 받는다. 
...
public class HelloServlet extends HttpServlet {
	...
	/**
	 * 3. service() 메소드
	 * 	 1) 클라이엔트가 요청할 때 자동으로 호출되는 메소드이다. 
	 ...
	 * 
	 * 4.doGet()
	 * 	1) GET 방식의 요청을 처리하는 메소드
	 *  2) 기본적으로 POST 방식의 요청도 처리하도록 구성되어 있다. 
	 *  3) GET방식으로 요청하는 방법 
	 *  	(1) <a href="/servlet/HelloServlet">
	 *      (2) <from method="get" action="/servlet/HelloServlet">
	 ...
	 *  3) POST 방식으로 요청하는 방법
	 *      (1) <from method="post" action="/servlet/HelloServlet">
	 *      (3) location.href = '/servlet/HelloServlet'
	 *      (4) window.open('/servlet/HelloServlet', '', '')
	 *      (5) $.ajax({
	 *           type : 'post',
