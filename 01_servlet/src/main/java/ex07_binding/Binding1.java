...
public class Binding1 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 ...
		 * 4. 저장영역
		 * 	1) ServletContext : 컨텍스트(프로젝트, 애플리케이션) 종료 값까지 값을 저장할 수 있다. 애플리케이션이 실행중이라면 어디서든 값을 접근할 수 있다. 
		 * 	2) HttpSession    : 세션 종료 전까지 값을 저장할 수 있다. 웹 브라우저가 실행중이라면 어디서든 값에 접근할 수 있다. (시간 지정 가능)
		 *  3) HttpServletRequest : 요청 종료 전까지 값을 저장할 수 있다. 등답전이라면 값에 접근할 수 있다. (1회용)
		...
		//HttpServeltRequest 영역에 값을 저장하기(이미 매개변수에 선언되어 있다)
